...
$(window).on('scroll', function () {
  ...
  sections.forEach(id => {
    ...
    if (scrollTop >= sectionTop - 300 && scrollTop < sectionTop + sectionHeight - 300) {
      if (id === '#skill') {
        activeId = '#about';
        $('#skill_wrap').addClass('on3');
      } else {
        activeId = id;
        $('#skill_wrap').removeClass('on3');
      }
  ...
  if (activeId) {
    ...
    //             // figure 'on3' 토글 (about, project 등 피규어가 있는 섹션)
    //             $(`${id} article figure`).addClass('on3');

    //             // skill_wrap은 skill 섹션에서만 on3 토글
    //             if (id === '#skill') {
    //                 $('#skill_wrap').addClass('on3');
    //             } else {
    //                 $('#skill_wrap').removeClass('on3');
    ...
    //             $(`${id} article`).removeClass('on');
    //             $(`${id} article figure`).removeClass('on3');
    //             $(`#nav_list a[href="${id}"]`).removeClass('active');
