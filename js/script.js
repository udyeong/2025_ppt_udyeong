$(document).ready(function () {
    ...
    let sections = ['#home', '#about', '#skill', '#project', '#design', '#contact'];
    let scrolling = false;
    // 각 섹션의 id를 배열로 잡고, 스크롤링을 기본 거짓으로 선언해 놓기

    $(window).on('scroll', function () {
        if (scrolling) return;
        // 실행 시 스크롤링이 진행중(true)이면 멈춰!
        ...
        sections.forEach(id => {
            ...
            if (scrollTop >= sectionTop - 350 && scrollTop < sectionTop + sectionHeight - 350) {
                if (id === '#skill') {
                    activeId = '#about';
                    $('#skill ul').addClass('on3');
                } else {
                    activeId = id;
                    $('#skill ul').removeClass('on3');
                }
                $(`${id} article`).addClass('on');
                $(`${id} article figure`).addClass('on3');
            } else {
                $(`${id} article`).removeClass('on');
                $(`${id} article figure`).removeClass('on3');
            }
        });
        // 각 섹션의 높이를 구해 클래스 추가로 이벤트 만들기
        $('#nav_list a').removeClass('active');
        if (activeId) {
            $('#nav_list a[href="' + activeId + '"]').addClass('active');
        }
    });
